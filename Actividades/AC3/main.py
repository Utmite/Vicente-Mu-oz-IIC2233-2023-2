...
class Jugador(Thread):
    ...
    def __init__(
        ...
        self.rivales: List[Jugador] = list()
    ...
    def intentar_capturar_bandera(self) -> None:
        # Completar
        handed = self.lock_bandera.acquire(False)
        if handed == True and self.bandera.nombre_dueño == None:
    ...
    def intentar_robar_bandera(self) -> bool:
        ...
        if random() < self.PROBABILIDAD_ROBAR:
            with self.lock_bandera:
                self.capturar_bandera()
                print("Se ha robado la bandera")
            return True
        else:
            return False
    ...
    def correr_segunda_mitad(self) -> bool:
        with self.lock_carrera:
            if self.senal_fin.is_set():
                self._correr = False
                return False
            elif self.posicion == 100 and self.tiene_bandera == True:
                self.senal_fin.set()
                self._correr = False
                return True

            elif self.tiene_bandera == False:
                self.intentar_robar_bandera()
        self.avanzar()
        time.sleep(self.TIEMPO_ESPERA)

    def run(self) -> None:
        # Completar con la primera parte de este método
...
class Carrera(Thread):
    ...
    def entregar_ganador(self) -> None:
        ...
        if jugadores[2].puntaje != jugadores[1].puntaje:
            ...
            print(
                msg.format(jugadores[1].name, jugadores[2].name, jugadores[1].puntaje)
            )
...
if __name__ == "__main__":
    ...
    j1 = Jugador("Zelda", bandera, lock_bandera, senal_inicio, senal_fin, lock_carrera)
    j2 = Jugador("Kirby", bandera, lock_bandera, senal_inicio, senal_fin, lock_carrera)
    j3 = Jugador("Conan", bandera, lock_bandera, senal_inicio, senal_fin, lock_carrera)
