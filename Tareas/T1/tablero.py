from NotTablero import NotTablero
...
class Tablero:
    ...
    # NOTE: Funcion terminada pasa los test, (NO TOCAR SOLO INSPIRAR)
    @property
    def desglose(self) -> list:
        contador = {}
        for pos_fila, fila in enumerate(self.tablero):
            for pos_columna, columna in enumerate(fila):
                if columna == "--":
                    cantidad = contador.get("vacias", 0)
                    contador["vacias"] = cantidad + 1
                elif columna == "PP":
                    cantidad = contador.get("peones", 0)
                    contador["peones"] = cantidad + 1
                else:
                    cantidad = contador.get("explosivas", 0)
                    contador["explosivas"] = cantidad + 1
        return [
            contador.get("explosivas", 0),
            contador.get("peones", 0),
            contador.get("vacias", 0),
        ]
    ...
    def piezas_explosivas_invalidas(self) -> int:
        T = Tablero(copy.deepcopy(self.tablero))
        T.limpiar()

        invalidos = 0
        piezas = T.obtener_piezas_explosiva()

        for pieza in piezas:
            pieza: PiezaExplosiva

            if pieza.alcance > T.celdas_afectadas(pieza.fila, pieza.columna):
                invalidos = invalidos + 1
        return invalidos
    ...
    def reemplazar(self, nombre_nuevo_tablero: str) -> bool:
        ...
        tableros = list(map(lambda i: i.strip().split(","), lineas))
        tableros = dict(map(lambda i: (i[0], (int(i[1]), int(i[2]), i[3:])), tableros))

        tablero = tableros.get(nombre_nuevo_tablero, None)
        if tablero is None:
            return False
        filas, columnas, tablero_plano = tablero

        tablero_vacio = [[0 for x in range(columnas)] for y in range(filas)]

        pos = 0
        for x in range(filas):
            for y in range(columnas):
                elemento = tablero_plano[pos]
                tablero_vacio[x][y] = elemento
                pos += 1
    ...
    # TODO: Completar este método
    def solucionar(self, T:NotTablero = None) -> list:
        if T is None:
            T = NotTablero(copy.deepcopy(self.tablero))  

        if T.piezas_explosivas_invalidas == 0:
            return copy.deepcopy(T.tablero)

        for x in range(T.filas):
            for y in range(T.columnas):

                if not (T.tablero[x][y] in "----"):
                    continue

                T.tablero[x][y] = "PP"

                if T.peones_invalidos != 0:
                    T.tablero[x][y] = "--"
                    continue
                
                T2 = NotTablero(copy.deepcopy(T.tablero))
                sol = self.solucionar(T2)
